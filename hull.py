...
Key representation is through transverse sections at a set of stations.
Stations are numbered from bow (lowest) to stern (highest).
"""
...
class Hull(object):
    ...
    def __init__(self, filename=None):
        """Initialize Hull object, optionally read from filename."""
        self.title = "Hull"
    ...
    def summary_stats(self):
        """String summarizing hull statistics."""
        s = '\nSummary dimensions (outside):\n'
        s += 'Length: %s = %s\n' % (format_feet_inches(self.outside_length), format_inches(self.outside_length))
        s += 'Beam: %.1f"(max) %.1f"(sheer)\n' % (self.outside_max_beam, self.outside_sheer_beam)
        s += 'Center depth: %.1f"\n' % (self.outside_center_depth)
        s += 'Bow height: %.1f"\n' % \
            (self.bow_profile[0][1] - self.profile_height[self.mid_station] + self.hull_thickness)
        s += 'Stern height: %.1f"\n' % \
            (self.stern_profile[0][1] - self.profile_height[self.mid_station] + self.hull_thickness)
        s += 'Displacements:\n' + self.displacement_summary()
        s += '\n'
        s += 'Summary dimensions (inside/form):\n'
        s += 'Length: %s = %s\n' % (format_feet_inches(self.length()), format_inches(self.length()))
        s += 'Beam: %.1f"(max) %.1f"(sheer)\n' % (self.max_width * 2.0, self.beam_sheer())
        s += 'Centre depth: %.1f"\n' % (self.depth(self.mid_station))
        s += 'Bow height: %.1f"\n' % (self.bow_profile[0][1] - self.profile_height[self.mid_station])
        s += 'Stern height: %.1f"\n' % (self.stern_profile[0][1] - self.profile_height[self.mid_station])
        s += '\n'
        s += 'Construction dimensions:\n'
        s += 'Gunwale length: %s\n' % (format_feet_inches(self.sheer_length() + self.sheer_length_extension_through_thickness()))
        outside_circ = self.circumference() + (2.0 * self.hull_thickness)
        s += 'External circumference: %s\n' % (format_inches(outside_circ))
        # Estimate of cedar strips
        s += 'Hull outside area estimate: %.1f sq ft\n' % (self.hull_outside_area() / 144.0)
        s += 'Strip estimate (5/8" strips, 20%% over): %.1f ft\n' % ((self.hull_outside_area() / 144.0) * 12.0 * 8.0 / 5.0)
        # http://www.clcboats.com/shoptips/strip_planking/how-many-cedar-strips-will-i-need.html
        s += 'CLC strip estimate (5/8" strips, 20%% over): %.1f ft\n' % (self.outside_length * outside_circ * 18.0 / 144.0)
        return(s)
